'''app.py provides functions for inventory monitoring, demand
consumption analysis, EOQ calculation, reorder point determination,
supplier evaluation, purchase order management, forecasting support,
...
def load_data():
    ...
    if "lead_time_days" not in materials.columns:
        preferred = supplier_map[supplier_map["preferred"]][["material_id", "lead_time_days"]]
...
def get_dynamic_eoq(dataset, material_id):
    '''EOQ from actual 12-month consumption; falls back to static EOQ if no data'''
    today       = pd.Timestamp.today().normalize()
    cutoff      = today - pd.DateOffset(months=12)
    sales_12m   = dataset["sales"][dataset["sales"]["date"] >= cutoff]
    daily       = get_daily_consumption(sales_12m, dataset["bom"])
    mat_daily   = daily[daily["material_id"] == material_id]
    total_12m   = float(mat_daily["quantity"].sum())

    m = dataset["materials"][dataset["materials"]["material_id"] == material_id].iloc[0]
...
# ── MONTHLY CONSUMPTION ───────────────────────────────────────
def get_monthly_consumption(dataset):
...
# ══════════════════════════════════════════════════════════════
def get_static_snapshot(dataset):
    '''Inventory status from static formulas — avg_daily = annual/365, nominal lead time, no ML'''
    materials    = dataset["materials"]
    supplier_map = dataset["supplier_map"]
    pos          = dataset["purchase_orders"]
    today        = pd.Timestamp.today().normalize()
    stock_dict   = get_current_stock(dataset)
...
def get_dynamic_snapshot(dataset, fc_models, fc_scalers,
    ...
    sales_12m     = dataset["sales"][dataset["sales"]["date"] >= cutoff_12m]
    daily_12m_all = get_daily_consumption(sales_12m, dataset["bom"])
    ...
    for _, m in materials.iterrows():
        ...
        if lt_model is not None and lt_scaler is not None and lt_le is not None:
            try:
                ...
                sup_details = dataset["suppliers"][
                    dataset["suppliers"]["supplier_id"] == supplier_id
                ].iloc[0]
...
# ── ALERT GENERATOR ───────────────────────────────────────────
def get_alerts(snapshot_df, dataset):
    ...
    triggered = snapshot_df[snapshot_df["reorder_triggered"]].copy() #changes
...
def mark_received(po_id, material_id, qty_received, receipt_date_str):
    ...
    try:
        ...
        raise ValueError(f"Material {material_id} not found in 07_inventory.csv") from exc
