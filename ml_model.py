'''main.py mainly used for training the linear and 
ridge model and saved output in pfl file'''
...
def train_lead_time_model(dataset):
    '''raining Model 1: Lead Time Prediction (Linear Regression)'''
    print("Training Model 1: Lead Time Prediction (Linear Regression)...")

    po  = dataset["purchase_orders"].copy()
    sup = dataset["suppliers"].copy()
...
def train_demand_forecast(dataset):
    ...
    monthly = get_monthly_consumption(dataset)
    ...
    for mid in monthly["material_id"].unique():
        ...
        print(f"last_features saved: {mid} l1={l1_val:.1f} l2={l2_val:.1f} l3={l3_val:.1f}")
